	...
	function renderCrewVP(): React.ReactNode {
		...
		return (
			<div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '.3em', flexWrap: 'nowrap' }}>
				...
				<img src={`${import.meta.env.VITE_ASSETS_URL}atlas/victory_point_icon.png`} style={{ height: '1em' }} className='invertibleIcon' />
			</div>
		);
	}
		...
		if (exclusiveBonus === 0) return <></>;
			...
			<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
				<div style={{ display: 'flex', alignItems: 'center', gap: '.3em' }}>
					<img src={`${import.meta.env.VITE_ASSETS_URL}captains_bridge_antimatter.png`} style={{ height: '1em' }} />
			...
			if (traits?.length) {
				return (
					<div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: '.5em' }}>
						<img src={`${import.meta.env.VITE_ASSETS_URL}atlas/icon_shipability_overcharge.png`} style={{ height: '1em' }} className='invertibleIcon' />
