...
class CommonCrewData extends Component<CommonCrewDataProps> {
		...
		if (roster && roster.length > 0) {
			panels.push({
				...
				content: {
					content: (
						<div style={{ paddingBottom: "1em" }}>
							{this.renderOtherRanks(crew, roster)}
						</div>
					),
				},
			});
			...
			<React.Fragment>
											...
											{crew.skill_data.map((sk: any, idx: number) => (
												<Segment key={idx}>
													...
													>
														...
														<CrewStat
															skill_name="security_skill"
															data={sk.base_skills.security_skill}
															scale={0.75}
														/>
														<CrewStat
															skill_name="command_skill"
															data={sk.base_skills.command_skill}
															scale={0.75}
														/>
														<CrewStat
															skill_name="diplomacy_skill"
															data={sk.base_skills.diplomacy_skill}
															scale={0.75}
														/>
														<CrewStat
															skill_name="science_skill"
															data={sk.base_skills.science_skill}
															scale={0.75}
														/>
														<CrewStat
															skill_name="medicine_skill"
															data={sk.base_skills.medicine_skill}
															scale={0.75}
														/>
														<CrewStat
															skill_name="engineering_skill"
				...
				{crewDemands && (
					<p>
						...
						<span style={{ display: "inline-block" }}>
							<img
								src={`${process.env.GATSBY_ASSETS_URL}currency_sc_currency_0.png`}
								height={16}
							/>
						</span>{" "}
				...
				{crew.cross_fuse_targets &&
					"symbol" in crew.cross_fuse_targets &&
					crew.cross_fuse_targets.symbol && (
						<p>
							Can cross-fuse with{" "}
							<Link to={`/crew/${crew.cross_fuse_targets.symbol}/`}>
								{crew.cross_fuse_targets.name}
							</Link>
							.
						</p>
					)}
				...
				<p>
					<b>Date added: </b>
					{new Date(crew.date_added).toLocaleDateString("en-US")} (
					<b>Obtained: </b>
					{crew.obtained})
				</p>
	...
	rosterComparisonTitle(crew: CrewMember, roster: PlayerCrew[]) {
		...
		const rankHandler = (prefix: string) => {
			let [name, rank] = Object.entries(crew.ranks)
				...
				.map(([k, v]) => [
					k,
					roster.filter((c) => c.ranks[k] < crew.ranks[k]).length + 1,
				])
				.sort(([k1, v1], [k2, v2]) => (v1 as number) - (v2 as number))[0];
			return [(name as string).slice(2).replace("_", "/"), rank];
		};
			...
			let rank =
				roster.filter(
					(c) =>
						...
						crew.ranks.voyTriplet.rank > c.ranks.voyTriplet.rank
				).length + 1;
	...
	renderOtherRanks(crew: CrewMember, roster: CrewMember[] | false = false) {
		...
		const rankHandler = (rank: string) =>
			roster
				? roster.filter(
					(c) => c.ranks[rank] && crew.ranks[rank] > c.ranks[rank]
				).length + 1
				: crew.ranks[rank];

		const tripletHandler = (rank: string) =>
			roster
				? roster.filter(
					(c) =>
						c.ranks[rank] &&
						c.ranks[rank].name == crew.ranks[rank].name &&
						crew.ranks[rank].rank > c.ranks[rank].rank
				).length + 1
					...
					<Statistic key={rank}>
						...
						<Statistic.Value>
							{rankLinker(
								...
								rankHandler(rank),
								crew.symbol,
								CONFIG.SKILLS_SHORT.find((c) => c.short === rank.slice(2))
									?.name ?? "",
								"descending"
							)}
						</Statistic.Value>
...
const rankLinker = (
	...
	return (
		<Link to={url} onClick={(event) => clickLink(event)}>
			{rank}
		</Link>
	);
