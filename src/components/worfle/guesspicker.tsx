			...
			<React.Fragment>
				<Image>
					<div style={{ opacity: isGuessed ? .5 : 1 }}>
						<img src={`${import.meta.env.VITE_ASSETS_URL}${crew.imageUrlPortrait}`} style={{ maxHeight: '72px' }} />
					</div>
					{isGuessed && (
						...
						<Label>
							<div>
								{crew.skill_order.map(skill => (
									<img key={skill} src={`${import.meta.env.VITE_ASSETS_URL}atlas/icon_${skill}.png`} style={{ height: '1em' }} />
								))}
							</div>
						</Label>
					)}
