...
export const AvatarView = (props: AvatarViewProps) => {
    ...
    function prepareCrew(gen_item?: BasicItem) {
        ...
        if (gen_item) {
            ...
            if (!src) src= `${process.env.REACT_ASSETS_URL}${crew.imageUrlPortrait}`;
            if (!playerData) {
            ...
            if ((showMaxRarity || crew.immortal) && crewBackground === 'rich') {
                ...
                if (kwip?.length === 4 && kwip?.every((qs) => typeof qs === 'number' ? !!qs : !!qs[1])) {
                    imgStyle.backgroundImage = `url(${process.env.REACT_ASSETS_URL}collection_vault_vault_item_bg_postimmortalized_256.png)`;
                }
                else {
                    imgStyle.backgroundImage = `url(${process.env.REACT_ASSETS_URL}collection_vault_vault_item_bg_immortalized_256.png)`;
                }
    ...
    function prepareItem(gen_item?: BasicItem) {
        ...
        if (gen_item) {
            ...
            if (item && citem) {
                ...
                if (!src) {
                    if (item.imageUrl) {
                        ...
                        src = `${process.env.REACT_ASSETS_URL}${getIconPath(item.icon, true)}`;
                    }
            ...
            if (item && !src) src = `${process.env.REACT_ASSETS_URL}${item.imageUrl}`;
            gen_item = item;
    ...
    function prepareShip(gen_item?: BasicItem) {
        ...
        if (gen_item) {
            ...
            if (props.useSchematicsIcon && cship) {
                if (!src) src = `${process.env.REACT_ASSETS_URL}${cship.icon?.file.slice(1).replace('/', '_')}.png`;
            }
            else if (ship) {
                if (!src) src = `${process.env.REACT_ASSETS_URL}${ship.icon?.file.slice(1).replace('/', '_')}.png`;
            }
            gen_item = ship;
            if (gen_item && !gen_item.max_rarity) {
                gen_item.max_rarity = gen_item.rarity;
