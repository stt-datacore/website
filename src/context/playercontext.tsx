...
export const PlayerProvider = (props: DataProviderProperties) => {
	...
	const { playerLanguage, setPlayerLanguage } = React.useContext(LocalizedContext);
	...
	React.useEffect(() => {
		...
		if (playerLanguage !== input.player.lang) {
			if (input.player.lang === 'es') {
				setPlayerLanguage('sp');
			}
			else {
				setPlayerLanguage(input.player.lang as SupportedLanguage);
			}
			return;
		}
		...
		setLoaded(true);
	}, [input, crew, ship_schematics, playerLanguage]);
