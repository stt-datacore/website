...
export const EnergyLogContextProvider = (props: IEnergyLogContextProvider) => {
    ...
    async function searchRemote(startDate?: Date, endDate?: Date) {
        ...
        const dbid = playerData.player.dbid;
        let url = `${import.meta.env.VITE_DATACORE_URL}api/playerResources?dbid=${dbid}`;
        if (startDate) {
    ...
    async function logToRemote(entry: EnergyLogEntry) {
        if (!playerData) return false;
        let dbid = playerData.player.dbid;
        let url = `${import.meta.env.VITE_DATACORE_URL}api/playerResources`;
        let postBody = {
    ...
    async function updateRemote(entries: EnergyLogEntry[]) {
        if (!playerData) return false;
        let dbid = playerData.player.dbid;
        let url = `${import.meta.env.VITE_DATACORE_URL}api/playerResourcesBatch`;
    ...
    async function clearRemote() {
        if (!playerData) return false;
        let dbid = playerData.player.dbid;
        let url = `${import.meta.env.VITE_DATACORE_URL}api/clearPlayerResources`;
        let postBody = { dbid };
         return fetch(url, {
            method: "POST",
            body: JSON.stringify(postBody),
