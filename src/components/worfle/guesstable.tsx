...
export const GuessTable = () => {
	...
	function renderLoserRow(): React.ReactNode {
...
const GuessRow = (props: GuessRowProps) => {
			...
			<Table.Cell style={styleCell(evaluatedGuess.variantEval)}>
				<div style={{ display: 'flex', flexDirection: 'column', rowGap: '.5em' }}>
					...
					<div style={{ display: 'flex', alignItems: 'center', columnGap: '1em' }}>
						<div>
							<img src={`${import.meta.env.VITE_ASSETS_URL}${evaluatedGuess.crew.imageUrlPortrait}`} style={{ maxHeight: '72px' }} />
						</div>
	...
	function renderSkillCell(index: number): React.ReactNode {
		...
		return <img alt={skill} src={`${import.meta.env.VITE_ASSETS_URL}atlas/icon_${skill}.png`} style={{ height: '2em' }} />;
	}
	...
	function renderFavoriteToggle(crew: IRosterCrew): React.ReactNode {
	...
	function renderHints(): React.ReactNode {
