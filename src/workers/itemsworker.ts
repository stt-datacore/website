...
const ItemsWorker = {
	splitCrew: (crew: (PlayerCrew | CrewMember)[]) => {
		...
		if (process.env.NODE_ENV === 'development') cores = 1;
		const result = {
			cores,
	...
	paraDispatch: async (config: ParaDemandConfig): Promise<ICrewDemands | undefined> => {
		const worker = new Worker(new URL('./parademand.ts', import.meta.url), { type: 'module' });
			...
			fetch('/structured/items.json')
				...
				.then((items: EquipmentItem[]) => {
					...
					if (!!playerData?.player?.character?.crew?.length && !!data?.length) {
						...
						Promise.all(workers)
							.then(results => results.filter(f => f !== undefined))
							.then((demandres) => {
