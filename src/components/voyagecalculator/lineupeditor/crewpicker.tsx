...
export const AlternateCrewPicker = (props: AlternateCrewPickerProps) => {
	...
	function renderReplacement(): React.ReactNode {
		...
		const seat: React.ReactNode = (
			<React.Fragment>
				<img
					src={`${process.env.VITE_ASSETS_URL}atlas/icon_${targeting.slot.skill}.png`}
					style={{ height: '1em' }}
	...
	function renderCrewAssignment(crew: IAlternateCrewData): React.ReactNode {
		...
		return <img src={`${process.env.VITE_ASSETS_URL}atlas/icon_${slottedSkills[skillSlot]}.png`} style={{ height: '1.1em', verticalAlign: 'middle' }} className='invertibleIcon' />
	}
				...
				<Message	/* Only showing viable SEAT alternates */>
					...
					<span style={{ marginRight: '1em' }}>
						{tfmt('voyage.editor.targeting_message', {
							seat: <><img src={`${process.env.VITE_ASSETS_URL}atlas/icon_${targeting.slot.skill}.png`} style={{ height: '1.1em', verticalAlign: 'middle' }} className='invertibleIcon'/> {t(`voyage.seats.${targeting.slot.symbol}`)}</>
						})}
					</span>
					<Button	/* Cancel */
