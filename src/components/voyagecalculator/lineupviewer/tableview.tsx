...
export const TableView = () => {
	...
	React.useEffect(() => {
		(async () => {
			...
			for (let a of assignments) {
				let imgurl = `${process.env.REACT_ASSETS_URL}${a.crew.imageUrlPortrait}`;
						...
						<Table.Cell width={2} className='iconic' style={{ fontSize: `${compact ? '1em' : '1.1em'}` }}>
							{ship.traits?.includes(voyageConfig.ship_trait) &&
								<span style={{ cursor: 'help' }}>
									<Popup content='+150 AM' mouseEnterDelay={POPUP_DELAY} trigger={<img src={`${process.env.REACT_ASSETS_URL}atlas/icon_antimatter.png`} style={{ height: '1em', verticalAlign: 'middle' }} className='invertibleIcon' />} />
								</span>
							}
						</Table.Cell>
						{configSource === 'player' && (
	...
	function renderSkillAssignments(index: number, for_export?: boolean): React.ReactNode {
						...
						return (
							<Table.Row key={idx} style={{
								...
								{!!for_export &&
									(<React.Fragment>
										<Table.Cell>
											<img src={`${process.env.REACT_ASSETS_URL}${crew.imageUrlPortrait}`} style={{height: '48px'}} />
											{/* <AvatarView
												src={imgdata}
												mode='crew'
												item={crew}
								...
								<Table.Cell width={1} className='iconic' style={{ fontSize: `${compact ? '1em' : '1.1em'}` }}>
									{!for_export && isQuipped(crew) && (
										<Popup wide content={renderKwipmentBonus((crew.kwipment as number[][]).map(q => typeof q === 'number' ? q : q[1]), globalContext.core.items, crew.kwipment_prospects, t)} mouseEnterDelay={POPUP_DELAY} trigger={
											<span style={{ cursor: 'help' }}>
												<img src={`${process.env.REACT_ASSETS_URL}atlas/ContinuumUnlock.png`} style={{ height: '1em', verticalAlign: 'middle' }} className='invertibleIcon' />
											</span>
										} />
									)}
									{!!for_export && isQuipped(crew) && (
	...
	function renderTraitBonus(crew: PlayerCrew, trait: string, for_export?: boolean): React.ReactNode {
		...
		return (
			<Popup
				...
				trigger={
					<span style={{ cursor: 'help' }}>
						<img src={`${process.env.REACT_ASSETS_URL}atlas/icon_antimatter.png`} style={{ height: '1em', verticalAlign: 'middle' }} className='invertibleIcon' />
					</span>
				}
			/>
		);
	...
	function renderVPBonus(crew: PlayerCrew, for_export?: boolean): React.ReactNode {
		...
		return (
			<Popup content={bonusText} mouseEnterDelay={POPUP_DELAY} trigger={
				<span style={{ cursor: 'help' }}>
					<img src={`${process.env.REACT_ASSETS_URL}atlas/victory_point_icon.png`} style={{ height: '1em', verticalAlign: 'middle' }} className='invertibleIcon' />
				</span>
			} />
		);
	}
