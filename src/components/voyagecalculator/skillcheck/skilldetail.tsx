		...
		<React.Fragment>
			<Message attached style={{backgroundColor: highlighted ? 'forestgreen' : undefined, cursor: 'pointer' }}
				...
				<Message.Header>
					...
					<Image floated='right' src={`${import.meta.env.VITE_ASSETS_URL}atlas/icon_${currentData.skill}.png`} style={{ height: '1.5em' }} />
				</Message.Header>
			</Message>
			<Segment attached>
			...
			<div style={{ display: 'flex', alignItems: 'center', columnGap: '.5em' }}>
				{pairedSkills.map(pairedSkill => (
					<span key={pairedSkill.skill}>
						...
						<img src={`${import.meta.env.VITE_ASSETS_URL}atlas/icon_${pairedSkill.skill}.png`} style={{ height: '1.1em', verticalAlign: 'middle' }} />
					</span>
				))}
			</div>
		);
