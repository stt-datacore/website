    ...
    const poolList = filteredShips?.map((c) => (
		{
			...
			value: c.id,
			image: { avatar: true, src: `${process.env.REACT_ASSETS_URL}${c.icon?.file.slice(1).replace('/', '_')}.png` },
			text: c.name,
			...
			content: (
				<div style={{...flexRow, justifyContent: 'flex-start', gap: '1em', display: 'inline-flex', flexWrap: 'wrap'}}>
					{/* <img src={`${process.env.REACT_ASSETS_URL}${c.icon?.file.slice(1).replace('/', '_')}.png`} style={{height: '24px'}} /> */}
					<div style={{width: '10em'}}>
						{c.name}
					</div>
					{!!props.showStaff && c.battle_stations?.map((bs, i) => {
						return <img key={`${bs.crew?.symbol}_staff_${c.id}_${i}`} src={`${process.env.REACT_ASSETS_URL}${bs.crew?.imageUrlPortrait}`} style={{height:'24px'}} />
					})}
				</div>
			)
	...
	const poolList = availableAbilities?.map((c) => (
		{
			...
			//image: { avatar: true, src: `${process.env.REACT_ASSETS_URL}atlas/icon_${c}.png` },
		...
		<React.Fragment>
			<Menu fluid={fluid !== false}>
				{availableSeats.map((c, key) => (
					...
					>
						<img src={`${process.env.REACT_ASSETS_URL}atlas/icon_${c}.png`} style={{width: "1em"}} />
					</Menu.Item>)
				)}
			</Menu>
		</React.Fragment>
