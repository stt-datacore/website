...
export const HelperMode = (props: HelperModeProps) => {
	...
	return (
		<Message icon>
			<img src={`${process.env.REACT_ASSETS_URL}/atlas/shuttle_icon.png`} style={{ width: '3em', marginRight: '1em' }} />
			<Message.Content>
	...
	function renderStandardOnly(): React.ReactNode {
		let label: string = '';
	...
	function renderStandardOption(): React.ReactNode {
		let label: string = '';
		if (activeShuttles.length > 0)
	...
	function renderEventOption(eventData: IEventData): React.ReactNode {
		let label: string = '';
