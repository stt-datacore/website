...
export interface ShipStationProspectsProps {
    pageId: string;
...
export const ShipStationProspects = (props: ShipStationProspectsProps) => {
    const globalContext = React.useContext(GlobalContext);
    const { t } = globalContext.localized;

    const dbid = globalContext.player.playerData?.player.dbid ?? '';

    const { pageId, rarities, setAppliedProspects, targetGroup } = props;

    const [prospects, internalSetProspects] = useStateWithStorage<LockedProspect[]>(`${dbid}/${pageId}/staffer/locked_prospects`, [], { rememberForever: true });
    const [unownedOnly, setUnownedOnly] = useStateWithStorage<boolean>(`${dbid}/${pageId}/staffer/unowned_only`, false, { rememberForever: true });
    ...
    const corePool = globalContext.core.crew.filter(c => {
        let res = (!rarities?.length || rarities.includes(c.max_rarity));
        if (res && unownedOnly) {
