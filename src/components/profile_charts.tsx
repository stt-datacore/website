		...
		// <ErrorBoundary>
			...
			<ul>
				{factionRec?.map((e, idx) => (
					<li key={e.name} style={{margin: '0.5em 0'}}>
						<FactionFormatDisplay item={e} />
					</li>
				))}
			</ul>
							...
							{<Popup
								trigger={
									<Header
										...
										content={<div style={{marginBottom: '0.25em'}}>{entry.equipment.name}</div>}
...
const FactionFormatDisplay = (props: FactionFormatProps) => {
	...
	const names = factions.map(faction => faction.name).reduce((p, n) => {
		if (p) {
			return <div key={`faction_names_${item.name}`} style={{display: 'inline'}}><b>{p}</b> {t('global.or')} <b>{n}</b></div>;
		}
		else {
			return <div key={`faction_names_${item.name}`} style={{display: 'inline'}}><b>{n}</b></div>;
		}
	}, undefined as React.ReactNode | undefined);

	return (<div style={{...OptionsPanelFlexRow, justifyContent: 'flex-start', gap: '0.25em', margin: '1em 0'}}>
		{images}{' '}{names} &mdash; {item.count} {t('global.items')}
	</div>)
}
