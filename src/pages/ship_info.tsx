	...
	React.useEffect(() => {
		if (ships?.length && shipKey) {
			...
			if (newship) {
				...
				setCrewStations(newship?.battle_stations?.map(b => undefined as PlayerCrew | CrewMember | undefined) ?? []);
			}
			else {
				navigate("/ships");
			}
...
class ShipProfile extends Component<ShipProfileProps, ShipProfileState> {
	...
	componentDidMount(): void {
		...
		if (!this.state?.ships?.length || this._hasPlayer !== newHasPlayer) {			
			...
			this.setState({ ...this.state, ships: this.loadShips() });
		}
	}
	...
	componentDidUpdate() {
		...
		const ships = this.loadShips();
		const ship = ships.find(d => d.symbol === ship_key);

		if (ship) {
			if (ship.id !== this.state.activeShip?.id) {
