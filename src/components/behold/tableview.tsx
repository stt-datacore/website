...
export const TableView = (props: TableViewProps) => {
	...
	const tableConfig: ITableConfigRow[] = [
		...
		{ width: 1, column: 'totalChronCost', title: <><img src={`${import.meta.env.VITE_ASSETS_URL}atlas/energy_icon.png`} alt='Chroniton' style={{ height: '1em' }} /><br /><small>{tfmt('behold_helper.columns.build_cost')}</small></> },
		{ width: 1, column: 'craftCost', title: <><img src={`${import.meta.env.VITE_ASSETS_URL}currency_sc_currency_0.png`} alt='Credit' style={{ height: '1.1em' }} /><br /><small>{tfmt('behold_helper.columns.build_cost')}</small></> }
	];
	CONFIG.SKILLS_SHORT.forEach((skill) => {
		tableConfig.push({
			width: 1,
