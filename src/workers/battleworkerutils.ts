...
export function scoreLineUp(ship: Ship, crew: CrewMember[], mode: 'arena' | 'heal' | 'evade', max = 180) {
    ...
    if (mode === 'evade') {
        let sridx = crew.map(c => c.action).map((a, idx) => a.bonus_type === 1 ? ab + idx : -1).filter(f => f !== -1);
        ...
        return (a_eva * 4) + (a_att * 3) + a_acc + (hrs) + ((booms + crits) / 2) + (srs * 2);
    }
    if (mode === 'arena') {
...
export function getOverlap(ship: ShipAction, crew: ShipAction) {
    ...
    return {
        ...
        starts: bna.filter((f, x) => (!x && f) || (f && x && !bna[x-1])).length,
        map: bna
    }
}