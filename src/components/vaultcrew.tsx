...
function formatCrewStats(crew: PlayerCrew, playerData?: PlayerData, allCrew?: CrewMember[], buffs?: BuffStatTable): React.ReactNode {
	let skills = [] as React.ReactNode[];
...
class VaultCrew extends PureComponent<VaultCrewProps> {
	render() {
		...
		let star_reward = `${process.env.REACT_APP_ASSETS_URL}atlas/star_reward.png`;
		let star_reward_inactive = `${process.env.REACT_APP_ASSETS_URL}atlas/star_reward_inactive.png`;
		...
		let skillicons = [] as React.ReactNode[];
		...
		skills_sorted.forEach((s) => {
			let skillName = s[0];
			skillicons.push(<img key={skillName} src={`${process.env.REACT_APP_ASSETS_URL}atlas/icon_${skillName}.png`} style={iconStyle} />);
		});
		...
		if (crew.immortal > 0 || (crew.rarity === crew.max_rarity && crew.level === 100 && crew.equipment.length === 4)) {
			...
			portraitDivStyle.backgroundImage = `url(${process.env.REACT_APP_ASSETS_URL}collection_vault_vault_item_bg_immortalized_256.png)`;
		}
			...
			<div style={divStyle}>
				<div style={portraitDivStyle}>
					<Popup
						...
						trigger={<img src={`${process.env.REACT_APP_ASSETS_URL}${crew.imageUrlPortrait}`} style={{ width: '100%' }} />}
					/>
				...
				<div style={equipmentColumnStyle}>
					<div style={{ display: 'inline-block' }}>
						<img style={equipmentCellImg} src={`${process.env.REACT_APP_ASSETS_URL}${eqimgs[0]}`} />
					</div>
					<div style={{ display: 'inline-block' }}>
						<img style={equipmentCellImg} src={`${process.env.REACT_APP_ASSETS_URL}${eqimgs[1]}`} />
					</div>
					<div style={{ display: 'inline-block' }}>
						<img style={equipmentCellImg} src={`${process.env.REACT_APP_ASSETS_URL}${eqimgs[2]}`} />
					</div>
					<div style={{ display: 'inline-block' }}>
						<img style={equipmentCellImg} src={`${process.env.REACT_APP_ASSETS_URL}${eqimgs[3]}`} />
					</div>
				</div>
