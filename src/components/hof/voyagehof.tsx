...
import { HOFViewModes, NiceNamesForPeriod, VoyageHOFPeriod, VoyageHOFProps, VoyageStatEntry, VoyageStats } from "../../model/hof";
...
export const VoyageHOF = (props: VoyageHOFProps) => {
    const globalContext = React.useContext(GlobalContext);

    const { t, useT } = globalContext.localized;
    const { t: hof } = useT('hof');

    const { crew: allCrew } = globalContext.core;

    const { navigate, crew_symbols } = props;

    const [rankBy, setRankBy] = useStateWithStorage<RankMode>('voyage_hof/rank_by', 'voyages', { rememberForever: true });
    const [glanceDays, setGlanceDays] = useStateWithStorage('voyage_hof/glance_days', 28, { rememberForever: true });

    const [viewMode, setViewMode] = React.useState<HOFViewModes>('rankings');
    const [rawVoyages, setRawVoyages] = React.useState<RawVoyageRecord[] | undefined>();
    const [rows, setRows] = React.useState<{ stats: VoyageStatEntry[], key: VoyageHOFPeriod }[][]>([]);
    const [errorMessage, setErrorMessage] = React.useState<string | undefined>(undefined);
    const [crewSymbol, setCrewSymbol] = React.useState<string[] | undefined>(undefined);
    const [voyageStats, setVoyageStats] = React.useState<VoyageStats | undefined>(undefined);
    ...
    React.useEffect(() => {
        ...
        if (crewSymbol !== undefined) {
            setCrewSymbol(undefined);
        }
    }, [crew_symbols]);

    React.useEffect(() => {
        const crew = crewSymbol;
        if (!crew) {
            if (!!rawVoyages) {
                setRawVoyages(undefined);
            }
        ...
        loadCrew(crew);
        setViewMode('details');
    }, [crewSymbol, glanceDays]);

    if (!voyageStats || !allCrew) {
        return (
    ...
    const filteredCrew = getFilteredCrew();
    ...
    return (
        <div style={{display:'flex',flexDirection:'column',alignItems:'center', justifyContent: 'center'}}>
            ...
            <Step.Group fluid>
                <Step active={viewMode === 'rankings'} onClick={() => setViewMode('rankings')}>
                    <Step.Content>
                ...
                <Step active={viewMode === 'details'} onClick={() => setViewMode('details')}>
                    <Step.Content>
            ...
            {<div style={{display: viewMode === 'details' ? 'flex' : 'none', width: '100%', flexDirection: 'column', justifyContent: 'center', alignItems: 'center'}}>
                <div style={{
                    ...
                    <div style={{margin: "0.5em"}}>
                        ...
                        <Dropdown
                            ...
                            value={glanceDays}
                            onChange={(e, { value }) => setGlanceDays(value as number)}
                        />
                ...
                <HofDetails crewClick={clickCrew} hofState={{
                    voyageStats,
                    glanceDays,
                    rawVoyages,
                    rankBy,
                ...
                {!!crewSymbol?.length && !!rawVoyages &&
                    <Button style={{margin: "0.5em"}} onClick={(e) => setGlance(undefined)}>{t('hof.details.clear')}</Button>
                }
            ...
            >
                ...
                <Dropdown
                    ...
                    value={rankBy}
                    onChange={(e, { value }) => setRankBy(value as RankMode)}
                />
            </div>
    ...
    function setGlance(crew?: string[]) {
        crew = crew?.filter(f => !!f?.length && f !== 'undefined');
        if (crew?.length) {
            navigate(`/hall_of_fame/${crew.join(",")}`);
        }
        else {
            navigate(`/hall_of_fame`);
        }
    ...
    function clickCrew(crew?: string) {
        ...
        if (current?.length === 0) {
            ...
            navigate(`/hall_of_fame/${current?.join(",")}`);
        }
    }
    ...
    function loadCrew(crew: string[]) {
        ...
        fetch(`${process.env.REACT_APP_DATACORE_URL}api/voyagesByCrew?opand=1&crew=${crew.join(",")}&days=${glanceDays}`)
            ...
            .then((rawVoyages: RawVoyageRecord[]) => {
                ...
                setRawVoyages(rawVoyages);
                setViewMode('details');
            });
    }

    function loadVoyageHOF() {
        fetch(`${process.env.REACT_APP_DATACORE_URL}api/telemetry?type=voyage`)
            .then((response) => response.json())
            .then((voyageStats) => {
                const isMobile = typeof window !== 'undefined' && window.innerWidth < DEFAULT_MOBILE_WIDTH;
...
//         fetch(`${process.env.REACT_APP_DATACORE_URL}api/voyagesByCrew?opand=1&crew=${crew.join(",")}&days=${this.state.glanceDays}`)
//             .then((response) => response.json())
//             .then((rawVoyages: RawVoyageRecord[]) => {
//                 let codict = {} as { [key: string]: RawVoyageRecord }
//                 rawVoyages.forEach((voy) => {
