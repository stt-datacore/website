...
export const ContestantPicker = (props: ContestantPickerProps) => {
	...
	if (traits && traits.length > 0) {
		...
		columns.push(
			{
				id: 'crit_potential',
				title: <img src={`${process.env.REACT_APP_ASSETS_URL}atlas/crit_icon_gauntlet.png`} style={{ height: '1em', verticalAlign: 'middle' }} className='invertibleIcon' />,
				align: 'center',
		...
		return (
			<React.Fragment>
				{tfmt('voyage.contests.notes.simulate_contestant', {
					skills: <>{
						skills.map(skill => (
							<img key={skill} src={`${process.env.REACT_APP_ASSETS_URL}atlas/icon_${skill}.png`} style={{ height: '1.2em', verticalAlign: 'middle' }} className='invertibleIcon' />
						))
					}</>
				})}
				...
				{traitPool && traitPool.length > 0 && (
					<span>
						...
						{tfmt('voyage.contests.notes.crit_icon', {
							img: (
								<img src={`${process.env.REACT_APP_ASSETS_URL}atlas/crit_icon_gauntlet.png`} style={{ height: '1.1em', verticalAlign: 'middle' }} className='invertibleIcon' />
							)
						})}
					</span>
				)}
