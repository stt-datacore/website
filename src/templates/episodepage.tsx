			...
			screens = (
				<Comment.Group threaded>
					...
					{quest.screens.map((screen, idx) => (
						<Comment key={idx}>
							<Comment.Avatar src={`${process.env.REACT_APP_ASSETS_URL}${imageFileUrl(screen.speaker_image)}`} />
							<Comment.Content>
					...
					{quest.mastery_levels.map(ml => (
						<div key={ml.id}>
							<span style={{ display: 'inline-block' }}>
								<img src={`${process.env.REACT_APP_ASSETS_URL}atlas/${CONFIG.MASTERY_LEVELS[ml.id].imageUrl}.png`} height={14} />
							</span>
							<span>
		...
		return (
			<div>
				...
				{quest.intro && (
					<Comment.Group>
						<Comment>
							<Comment.Avatar src={`${process.env.REACT_APP_ASSETS_URL}${imageFileUrl(quest.intro.portrait)}`} />
							<Comment.Content>
		...
		return (
			...
			<Item.Group divided>
				{episode.quests.map((c, idx) => (
					<Item key={idx}>
						<Item.Image size='small' src={`${process.env.REACT_APP_ASSETS_URL}${this._getImageUrl(c)}`} />
