                ...
                {Array.from(selectedIds).map(selectedId => {
                    ...
                    return (
                        <Label key={crew.id} style={{ display: 'flex', flexWrap: 'nowrap', alignItems: 'center' }}>
                            <Image spaced='right' src={`${process.env.REACT_APP_ASSETS_URL}${crew.imageUrlPortrait}`} />
                            {crew.name}
    ...
    function renderGridCrew(datum: IEssentialData, isSelected: boolean): React.ReactNode {
        ...
        return (
            <React.Fragment>
                <Image>
                    <div style={{ opacity:  1 }}>
                        <img src={`${process.env.REACT_APP_ASSETS_URL}${crew.imageUrlPortrait}`} width='72px' height='72px' />
                    </div>
                    {isSelected && (
