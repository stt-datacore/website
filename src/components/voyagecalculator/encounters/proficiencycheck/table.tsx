...
export const ProficiencyTable = (props: ProficiencyTableProps) => {
	...
	const tableSetup = React.useMemo<IDataTableSetup>(() => {
		...
		const columns: IDataTableColumn[] = [
			...
			{
				id: 'crit_potential',
				title: <img src={`${process.env.REACT_APP_ASSETS_URL}atlas/crit_icon_gauntlet.png`} style={{ height: '1.1em', verticalAlign: 'middle' }} className='invertibleIcon' />,
				align: 'center',
	...
	function renderSkillHeader(skills: string[], crewCount: number): React.ReactNode {
		return (
			<span title={t('voyage.contests.n_viable_crew', { n: crewCount })}>
				{skills.map(skill => (
					<img key={skill} src={`${process.env.REACT_APP_ASSETS_URL}atlas/icon_${skill}.png`} style={{ height: '1.1em', verticalAlign: 'middle' }} className='invertibleIcon' />
				))}
				{` `}({crewCount})
			</span>
		);
